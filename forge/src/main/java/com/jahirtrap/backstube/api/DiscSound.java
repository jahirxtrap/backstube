...
public record DiscSound(
    ...
    private static final Codec<DiscSound> OBJECT_CODEC = RecordCodecBuilder.create(i -> i.group(
            Identifier.CODEC.optionalFieldOf("name").forGetter(DiscSound::name),
    ...
    private static final Codec<DiscSound> STRING_CODEC = Identifier.CODEC.xmap(
            id -> new DiscSound(Optional.of(id), 1.0F, 1.0F, true, 16),
            ds -> ds.name.orElseThrow()
    );

    /**
     * Codec accepting either an {@link Identifier} string (location-only
    ...
    public Identifier resolveName(Identifier discId) {
        return name.orElseGet(() -> Identifier.fromNamespaceAndPath(discId.getNamespace(), "records/" + discId.getPath()));
    }
}
