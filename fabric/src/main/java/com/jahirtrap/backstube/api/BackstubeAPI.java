...
public class BackstubeAPI {
    ...
    /**
     * Builds {@link Item.Properties} pre-configured to bind an item to the given
     * disc registry entry.
     * <p>
     * The returned properties have:
     * <ul>
     *   <li>{@code stacksTo(1)};</li>
     *   <li>delayed {@code backstube:disc} component &rarr; the holder for
     *       {@code diskKey};</li>
     *   <li>delayed {@code minecraft:rarity} component &rarr; the
     *       {@link BackstubeMusicDisc#rarity()} of the disc;</li>
     ...
     * specific disc &mdash; for example to give the disc its own recipe or
     ...
     * @see #createDisc(Identifier, Item.Properties, BackstubeMusicDisc)
     * @since 0.1.0
     */
    public static Item.Properties discProperties(ResourceKey<BackstubeMusicDisc> diskKey) {
        return new Item.Properties()
                .stacksTo(1)
                .delayedComponent(ModComponents.DISC, context -> context.getOrThrow(diskKey))
                .delayedComponent(DataComponents.RARITY, context -> context.getOrThrow(diskKey).value().rarity())
     ...
     * <p>
     * The returned item is registered into {@code minecraft:item} immediately;
     * assign it to a {@code static final} field so static initialization order
     * is preserved.
     *
     ...
     * @return the newly registered {@link Item}; never {@code null}
     * @see #createDisc(Identifier, Item.Properties, Function, BackstubeMusicDisc)
     * @since 0.1.0
     */
     ...
     * <p>
     * Identical to
     * {@link #createDisc(Identifier, Item.Properties, BackstubeMusicDisc)} but
     ...
     * @return the newly registered item, statically typed to {@code T}
     * @since 0.1.0
     */
    public static <T extends Item> T createDisc(Identifier id, Item.Properties properties, Function<Item.Properties, T> factory, BackstubeMusicDisc disc) {
        ...
        Item.Properties enriched = properties
                .delayedComponent(ModComponents.DISC, ctx -> ctx.getOrThrow(diskKey))
        ...
        return Registry.register(BuiltInRegistries.ITEM, itemKey, factory.apply(enriched.setId(itemKey)));
    }
    ...
    public static void applyDiscData(ItemStack stack, Holder<BackstubeMusicDisc> disc, HolderLookup.Provider provider) {
        ...
        provider.lookup(Registries.JUKEBOX_SONG)
                ...
                .ifPresent(song -> stack.set(DataComponents.JUKEBOX_PLAYABLE, new JukeboxPlayable(song)));
    }
