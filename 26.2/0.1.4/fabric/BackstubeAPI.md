...
| [`discProperties(ResourceKey<BackstubeMusicDisc>)`](#discproperties-resourcekey-backstubemusicdisc) | Builds `Item.Properties` pre-configured to bind an item to the given disc registry entry. |
...
| [`createDisc(Identifier, Item.Properties, BackstubeMusicDisc)`](#createdisc-identifier-item-properties-backstubemusicdisc) | Registers a code-based disc together with a custom `Item` bound to its registry entry. |
| [`createDisc(Identifier, Item.Properties, Function<Item.Properties, T>, BackstubeMusicDisc)`](#createdisc-identifier-item-properties-function-item-properties-t-backstubemusicdisc) | Registers a code-based disc together with a custom `Item` subclass. |
...
Builds `Item.Properties` pre-configured to bind an item to the given
disc registry entry.
...
- delayed `backstube:disc` component → the holder for
`diskKey`;

- delayed `minecraft:rarity` component → the
[`BackstubeMusicDisc.rarity()`](BackstubeMusicDisc.md#rarity) of the disc;

- `jukeboxPlayable` set to the matching jukebox song.

Use this when registering a dedicated item that should always represent a
specific disc — for example to give the disc its own recipe or
creative-tab entry. The disc data itself must still exist either as a JSON
file or as a code-registered entry via
...
> **See also:** [`createDisc(Identifier, Item.Properties, BackstubeMusicDisc)`](#createdisc-identifier-item-properties-backstubemusicdisc)
> 
> **Since** 0.1.0
...
### `createDisc(Identifier, Item.Properties, BackstubeMusicDisc)`

```java
public static Item createDisc(Identifier id, Item.Properties properties, BackstubeMusicDisc disc)
```
...
The returned item is registered into `minecraft:item` immediately;
assign it to a `static final` field so static initialization order
is preserved.
...
**Returns:** the newly registered `Item`; never `null`

> **See also:** [`createDisc(Identifier, Item.Properties, Function, BackstubeMusicDisc)`](#createdisc-identifier-item-properties-function-backstubemusicdisc)
> 
> **Since** 0.1.0

---

### `createDisc(Identifier, Item.Properties, Function<Item.Properties, T>, BackstubeMusicDisc)`

```java
public static <T extends Item> T createDisc(Identifier id, Item.Properties properties, Function<Item.Properties, T> factory, BackstubeMusicDisc disc)
```

Registers a code-based disc together with a custom `Item` subclass.

Identical to
[`createDisc(Identifier, Item.Properties, BackstubeMusicDisc)`](#createdisc-identifier-item-properties-backstubemusicdisc) but
...
**Returns:** the newly registered item, statically typed to `T`
